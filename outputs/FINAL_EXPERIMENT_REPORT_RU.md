...
### 4.4. Измеренная вычислительная эффективность gate

Временная гипотеза подтверждается внутри предлагаемой системы, если сравнивать
gated MRV с той же MRV-политикой при безусловном запуске graph expansion.
Средние значения по seed обучения 101/202/303 (`qwen3-embedding:0.6b`,
1 000 eval-вопросов на seed):
...
Таким образом, gate экономит 12.6–20.8% graph actions и 13.8–23.1% времени
graph-policy относительно always-on MRV. Для полного retrieval + policy
pipeline наблюдаемое снижение составляет 3.6–8.1%.

Это не является доказательством ускорения относительно внешнего GraphRAG:
измерения не включают offline graph/index construction и генерацию ответа LLM,
а полноценная внешняя система не запускалась на том же hardware. Кроме того,
упрощённый KG²RAG-style control имеет меньшую raw policy latency
(14.21/19.39/19.99 мс для HotpotQA/2Wiki/MuSiQue), чем gated MRV, поскольку
использует более простой selector; преимущество gated MRV над этим control
состоит в качестве evidence, а не в абсолютном времени policy.
